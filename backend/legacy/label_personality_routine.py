...
if os.path.isfile("personality_label_list"):
    print("Error: personality_label_list already exists.")
    ...
    with open("feature_list", "r") as features, open("personality_label_list", "a") as labels:
        for idx, line in enumerate(features):
            ...
            labels.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t" %
                         (str(trait_1), str(trait_2), str(trait_3), str(trait_4),
                          str(trait_5), str(trait_6), str(trait_7), str(trait_8)))
